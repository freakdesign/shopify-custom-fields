...
_.data("collections"),select=$("<select />",{"data-ref":a}).change(function(event){var t=$(this),v=t.val(),f=$('input[name="'+a+'"]').eq(0);if(v)f.val(v);else f.val(_.unescape_html(f.attr("data-val")))});if(c&&Object.keys(c.collections).length>1){var options='<option value="">Pick a collection</option>';for(var i=0,len=Object.keys(c.collections).length;i<len;i++)options+='<option value="'+c.collections[i].handle+'">'+c.collections[i].title+"</option>";select.html(options);return select}else _.notice("Error, invalid collection length",
true)}},key_check:function(a){var c={field:"text",type:false,limit:false};if(a.indexOf(_.data("key_pages"))>0);else if(a.indexOf(_.data("key_products"))>0);else if(a.indexOf(_.data("key_articles"))>0);else if(a.indexOf(_.data("key_collections"))>0);if(a.indexOf(_.data("marker_collection"))>-1){c.field="select";c.type="collection"}return c},init:function(){var freakdesign_main_button=$("<a>",{id:"freakdesign_main_button"}).text("Custom Fields ON").on("click",function(){_.fd_modal(true,'<p>Custom Fields is "honor-ware", which means that we trust each other to be nice. As the developer of it, I\'m committed to keep the tool something that\'s actually useful. By releasing new features and correcting possible bugs on a constant basis I can do just that, but I need your support. If you use it and intend to keep it, please sponsor its development by making a small <a target="_blank" href="http://shopify.freakdesign.com.au/#donate2">contribution</a>.</p><p>Developers should consider using <a target="_blank" href="http://shopify.freakdesign.com.au/#ShopifyFD">ShopifyFD</a> to create the metafields this tool requires.</p>',
