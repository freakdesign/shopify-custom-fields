...
custom_field_product_panel=$('<div class="row section"><div class="span6 section-summary"><h1>Product Custom Fields</h1><p>Product Custom Fields are unique to this product and only shown here when using the Custom Fields tool.</p></div><div class="span18 section-content"></div></div>'),insertafter=$("div.row.section.inventory").eq(0),save=$('input[data-addclass-btn-primary="product.canSave"]').eq(0);insertafter.after(custom_field_panel,custom_field_product_panel);var a=$("<a/>",{"class":"btn","href":"#"}).text("Save with Custom Fields").on("click",
function(){var inputs=$("div.custom_fields_box input, div.custom_fields_box textarea");if(inputs.length){var save_meta=function(i){var ci=inputs.eq(i),key=ci.attr("data-key"),value=ci.val(),ajax_type="POST",value_type=ci.attr("data-type"),metaJSON={"metafield":{"namespace":_.data("namespace"),"key":key,"value":value,"value_type":value_type}};var ajax_url=url,skip=false;if(value.length==0&&(ci.attr("data-mid")&&ci.attr("data-pid"))){ajax_type="DELETE";ajax_url="/admin/products/"+ci.attr("data-pid")+
"/metafields/"+ci.attr("data-mid")+".json"}else if(value.length==0)skip=true;if(!skip)$.ajax({type:ajax_type,url:ajax_url,dataType:"json",data:metaJSON,success:function(d){i++;if(i<inputs.length)save_meta(i);else save.click()},fail:function(d){_.notice("Error saving custom fields",true)}});else{i++;if(i<inputs.length)save_meta(i);else save.click()}};save_meta(0)}return false});save.after(a);_.data("fields",[]);$.ajax({type:"GET",url:"/admin/metafields.json",dataType:"json",success:function(d){var to_append=
"",makerow=function(){return false};for(var i=0,len=d.metafields.length;i<len;i++){var namespace=d.metafields[i].namespace,namespace_len=namespace.length;if(namespace.indexOf(_.data("namespace"))===0||namespace.indexOf(_.data("namespace_new"))===0){var key=d.metafields[i].key,myarray=_.data("fields"),meta_type=_.key_check(key);myarray.push(d.metafields[i].key);_.data("fields",myarray);key=key.replace(/\[([^\]]*)\]*/g,"");key=key.charAt(0).toUpperCase()+key.slice(1).replace(/-/g," ").replace(/_/g,
" ");to_append+='<div class="half">';if(d.metafields[i].value.length>1)to_append+='<label for="product-type">'+key+' <span class="note block">'+d.metafields[i].value+"</span></label>";else to_append+='<label for="product-type">'+key+"</label>";if(meta_type.field=="textarea")to_append+='<div class="half"><label for="product-type">'+key+'</label><textarea name="'+d.metafields[i].key+'" placeholder="'+d.metafields[i].key+'" data-key="'+d.metafields[i].key+'" data-type="'+d.metafields[i].value_type+'">'+
d.metafields[i].value+"</textarea></div>";else to_append+='<input data-cffield="'+meta_type.field+'" data-cftype="'+meta_type.type+'" name="'+d.metafields[i].key+'" placeholder="'+key+'" value="" data-key="'+d.metafields[i].key+'" data-type="'+d.metafields[i].value_type+'">';to_append+="</div>"}}if(!to_append.length)to_append=_.translate("no_fields");custom_field_panel.find("div.section-content").append('<div class="custom_fields_box clearfix"><div class="content">'+to_append+"</div></div>");custom_field_panel.find('input[data-cftype="collection"]').each(function(){var t=
$(this);t.after(_.build_select(t.attr("name")))});$.ajax({type:"GET",url:url,dataType:"json",success:function(d){var to_append="";for(var i=0,len=d.metafields.length;i<len;i++)if(d.metafields[i].namespace==_.data("namespace"))if($.inArray(d.metafields[i].key,_.data("fields"))<0){var key=d.metafields[i].key;var meta_type=_.key_check(key);key=key.replace(/\[([^\]]*)\]*/g,"");key=key.charAt(0).toUpperCase()+key.slice(1).replace(/-/g," ").replace(/_/g," ");if(meta_type.field==="textarea")to_append+='<div class="half"><label for="product-type">'+
key+'</label><textarea name="'+d.metafields[i].key+'" placeholder="'+d.metafields[i].key+'" data-key="'+d.metafields[i].key+'" data-type="'+d.metafields[i].value_type+'">'+d.metafields[i].value+"</textarea></div>";else to_append+='<div class="half"><label for="product-type">'+key+'</label><input name="'+d.metafields[i].key+'" placeholder="'+d.metafields[i].key+'" value="'+d.metafields[i].value+'" data-key="'+d.metafields[i].key+'" data-type="'+d.metafields[i].value_type+'"></div>'}else{var input=
$('input[data-key="'+d.metafields[i].key+'"],textarea[data-key="'+d.metafields[i].key+'"]');input.val(d.metafields[i].value).attr({"data-val":_.escape_html(d.metafields[i].value),"data-mid":d.metafields[i].id,"data-pid":_.data("omega")})}if(!to_append.length)to_append=_.translate("no_fields");custom_field_product_panel.find("div.section-content").append('<div class="custom_fields_box clearfix"><div class="content">'+to_append+"</div></div>")},error:function(d){_.notice(_.translate("error_get_metafields"),
true)}})},error:function(d){_.notice(_.translate("error_get_metafields"),true)}})},setup_page:function(a){},setup_article:function(a){},build_select:function(a){if(a){var c=_.data("collections"),select=$("<select />",{"data-ref":a}).change(function(event){var t=$(this),v=t.val(),f=$('input[name="'+a+'"]').eq(0);if(v)f.val(v);else{var resetVal=_.unescape_html(f.attr("data-val"));resetVal?f.val(resetVal):f.val("")}});if(c&&Object.keys(c.collections).length>1){var options='<option value="">Pick a collection</option>';
for(var i=0,len=Object.keys(c.collections).length;i<len;i++)options+='<option value="'+c.collections[i].handle+'">'+c.collections[i].title+"</option>";select.html(options);return select}else _.notice("Error, invalid collection length",true)}},key_check:function(a){var c={field:"text",type:false,limit:false};if(a.indexOf(_.data("key_pages"))>0);else if(a.indexOf(_.data("key_products"))>0);else if(a.indexOf(_.data("key_articles"))>0);else if(a.indexOf(_.data("key_collections"))>0);if(a.indexOf(_.data("marker_collection"))>
-1){c.field="select";c.type="collection"}if(a.indexOf(_.data("marker_textarea"))>-1){c.field="textarea";c.type="string"}return c},init:function(){var freakdesign_main_button=$("<a>",{id:"freakdesign_main_button"}).text("Custom Fields ON").on("click",function(){_.fd_modal(true,'<p>Custom Fields is "honor-ware", which means that we trust each other to be nice. As the developer of it, I\'m committed to keep the tool something that\'s actually useful. By releasing new features and correcting possible bugs on a constant basis I can do just that, but I need your support. If you use it and intend to keep it, please sponsor its development by making a small <a target="_blank" href="http://shopify.freakdesign.com.au/#donate2">contribution</a>.</p><p>Developers should consider using <a target="_blank" href="http://shopify.freakdesign.com.au/#ShopifyFD">ShopifyFD</a> to create the metafields this tool requires.</p>',
