...
custom_field_panel=d):"pages"===a.data("alpha")||"articles"===a.data("alpha")?e=$('header .header-right button[type="submit"]').eq(0):a.data("alpha");f.length?(a.setting("hideProductCustomFields")||f.after(custom_field_panel),e.length&&"Save"===e.val()&&e.val("Save (without Custom Fields)"),a.data("fields",[]),$.ajax({type:"GET",url:"/admin/metafields.json?limit=250",dataType:"json",success:function(b){for(var d="",k="",g=0,e=b.metafields.length;g<e;g++){var f=b.metafields[g].namespace,l=!1;0<a.data("whitelist").length&&
