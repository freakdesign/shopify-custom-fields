...
*/
(function(){if("undefined"!=typeof Shopify)if("undefined"!=typeof jQuery)if($("html.freakdesign_custom_fields").length)alert("Already loaded the tool?");else{var q=document,w=q.URL;q.documentElement.className+=" freakdesign_custom_fields";if(1<w.indexOf("myshopify.com/admin")){var a=function(){var q={no_fields:"No custom fields found",error_get_metafields:"Error getting product metafields",loaded:"Custom Field tool loaded"},t={debug:!1,alpha:!1,omega:!1,wait:1E3,showHelp:!0,whiteLabel:!1,lockToAlpha:!1,
...
var f=function(h,d){"undefined"===typeof h&&(h=0);if("undefined"===typeof d)a.notice("Type unknown",!0);else{var k=c.eq(h),g=k.attr("data-key"),u=k.val(),v="POST",l=k.attr("data-namespace"),p=k.attr("data-type"),m=!1,g={metafield:{namespace:l,key:g,value:u,value_type:p}};0===u.length&&k.attr("data-mid")&&k.attr("data-pid")?(v="DELETE",ajax_url="/admin/"+a.data("alpha")+"/"+k.attr("data-pid")+"/metafields/"+k.attr("data-mid")+".json",k.removeAttr("data-pid data-mid")):0===u.length?m=!0:ajax_url=b.url;
m?(h++,h<c.length?f(h,d):(e.attr("style","").removeClass("is-loading disabled"),a.notice("Custom fields saved"))):$.ajax({type:v,url:ajax_url,dataType:"json",data:g,success:function(b){h++;h<c.length?f(h,d):(e.attr("style","").removeClass("is-loading disabled"),a.notice("Custom fields saved"))},error:function(){a.notice("Error saving custom fields",!0);e.attr("style","").removeClass("is-loading disabled")}})}};f(0,d)}else a.notice("No fields to save",!0)}},setup_custom_fields:function(b){"undefined"===
typeof b&&(b=!1);var c="/admin/"+a.data("alpha")+"/"+a.data("omega")+"/metafields.json";$('<div class="row section"><div id="cf-summary" class="span6 section-summary"><h1>Custom Fields</h1><p>Custom Fields are created by your developer and shown here when using the Custom Fields tool.</p></div><div class="span18 section-content"></div></div>');var d=$('<section class="next-card"><div><div class="box box-details"><div class="box-header clearfix"><span class="box-header-title">Custom Fields</span><span id="cf-summary" class="box-header-action custom-fields-future-buttons"></span></div><div class="box-content"><div class="overflow-visible custom-fields-content"><p>Custom Fields are created by your developer and shown here when using the Custom Fields tool.</p><div class="next-card__section"></div></div></div></div></div></section>'),
...
a.data("showHelp")&&(custom_field_panel.find(".section-summary").append('<p>Refer to the <a target="_blank" href="'+a.data("help_file")+'">help file</a> for more info.</p>'),d.find(".custom-fields-content p").eq(0).after('<p>Refer to the <a target="_blank" href="'+a.data("help_file")+'">help file</a> for more info.</p>'));"customers"===a.data("alpha")?(f=$("#customer-notes").eq(0).parent(),custom_field_panel=d):"orders"===a.data("alpha")?(e=$("header .header-right ul").eq(1),f=$("section#order_notes").eq(0).parent(),
custom_field_panel=d):"pages"===a.data("alpha")||"articles"===a.data("alpha")?e=$('header .header-right button[type="submit"]').eq(0):a.data("alpha");f.length?(a.setting("hideProductCustomFields")||f.after(custom_field_panel),e.length&&"Save"===e.val()&&e.val("Save (without Custom Fields)"),a.data("fields",[]),$.ajax({type:"GET",url:"/admin/metafields.json",dataType:"json",success:function(b){for(var d="",k="",g=0,e=b.metafields.length;g<e;g++){var f=b.metafields[g].namespace,l=!1;0<a.data("whitelist").length&&
-1<$.inArray(f,a.data("whitelist"))&&(l=!0);var p=a.key_check(b.metafields[g].value+b.metafields[g].key);if(p.limit===a.data("alpha")||!1===p.limit)if(f===a.data("namespace")||f===a.data("namespace_new")||l){var m=b.metafields[g].key,n=a.data("fields");n.push(b.metafields[g].key);a.data("fields",n);m=a.remove_brackets(m);n='<div class="half">';n=1<b.metafields[g].value.length?n+('<label for="product-type"><span class="tooltip tooltip-bottom" style="display:inline-block;position:relative">'+m+'<span class="tooltip-container"><span class="tooltip-label">'+
b.metafields[g].namespace+"."+b.metafields[g].key+'</span></span></span><span class="note block">'+a.remove_brackets(b.metafields[g].value)+"</span></label>"):n+('<label for="product-type">'+m+"</label>");n="textarea"==p.field?n+('<textarea data-namespace="'+f+'" name="'+b.metafields[g].key+'" placeholder="'+a.remove_brackets(b.metafields[g].key)+'" data-key="'+b.metafields[g].key+'" data-type="'+b.metafields[g].value_type+'"></textarea>'):n+('<input data-namespace="'+f+'" data-cffield="'+p.field+
'" data-cftype="'+p.type+'" name="'+b.metafields[g].key+'" placeholder="'+a.remove_brackets(m)+'" value="" data-key="'+b.metafields[g].key+'" data-type="'+b.metafields[g].value_type+'">');n+="</div>";"undefined"!==typeof p.variant?k+=n:d+=n}}b=!0;d.length||(d='<div class="box warning">'+a.translate("no_fields")+"</div>",b=!1);custom_field_panel.find("div.next-card__section").append('<div class="custom_fields_box is-product clearfix fadein"><div class="content">'+d+"</div></div>");a.data("variantPanel",
k);if(b){custom_field_panel.find('input[data-cftype="collection"]').each(function(){var b=$(this);b.after(a.build_select(b.attr("name"),"collection"))});custom_field_panel.find('input[data-cftype="linklist"]').each(function(){});custom_field_panel.find('input[data-cftype="product"]').each(function(){});if("undefined"!==a.setting("datalists"))for(m in d=a.setting("datalists"),d)if(k=$('input[data-key="'+m+'"]'),k.length&&"false"===k.attr("data-cftype")){b=$("<datalist />",{id:"list-"+m});k.attr("list",
"list-"+m);var g=d[m],r;for(r in g)g.hasOwnProperty(r)&&(e=$("<option />",{value:g[r]}),b.append(e));k.after(b)}$.ajax({type:"GET",url:c,dataType:"json",success:function(b){for(var c=0,d=b.metafields.length;c<d;c++)l=!1,0<a.data("whitelist").length&&-1<$.inArray(b.metafields[c].namespace,a.data("whitelist"))&&(l=!0),(b.metafields[c].namespace===a.data("namespace")||b.metafields[c].namespace===a.data("namespace_new")||l)&&"global"!==b.metafields[c].namespace&&"description_tag"!==b.metafields[c].key&&
$('input[data-key="'+b.metafields[c].key+'"],textarea[data-key="'+b.metafields[c].key+'"]').val(b.metafields[c].value).attr({"data-mid":b.metafields[c].id,"data-pid":a.data("omega")})},error:function(b){a.notice(a.translate("error_get_metafields"),!0)}});m=$("<a />",{"class":"btn fadein btn-primary btn-slim"}).on("click",function(b){b.preventDefault();b={t:$(this),callback:!1,url:c};a.save_custom_fields(b)}).text("Save Custom Fields");r=custom_field_panel.find("div.section-summary");r.length?r.append(m):
custom_field_panel.find("span.custom-fields-future-buttons").append(m)}},error:function(b){a.notice(a.translate("error_get_metafields"),!0)}})):a.notice("Custom Fields error : setup_custom_fields() : target html not found",!0);"products"===b&&(b=$("<a />",{"class":"btn btn-slim customfield_btn tooltip tooltip-bottom tooltip-right-align fadein",href:"#"}).html('<span class="tooltip-container"><span class="tooltip-label">Show Custom Fields</span></span><svg style="vertical-align: middle" xmlns="http://www.w3.org/2000/svg" version="1.1" x="0px" y="0px" width="12px" height="12px" viewBox="0 0 12 12" enable-background="new 0 0 12 12" xml:space="preserve"><polygon fill="#D81965" points="0,0 0,12 10,12 10,11 11,11 11,10 12,10 12,0 "/><path fill="#FFFFFF" d="M7 9v1h1V9H7zM8 11h1v-1H8V11zM9 10h1V9H9V10zM10 9h1V8h-1V9zM1 11h1v-1H1V11zM2 10h1V9H2V10zM3 9h1V8H3V9zM1 6h10V5H1V6zM1 4h10V3H1V4zM1 1v1h10V1H1zM4 11v-1H3v1H4zM1 8v1h1V8H1z"/></svg>').on("click",
...
a.setting("datalists");for(var l in b)if(e=c.find('input[data-key="'+l+'"]'),1===e.length){if("false"===e.attr("data-cftype")){f=$("<datalist />",{id:"list-"+l});e.attr("list","list-"+l);var h=b[l],s;for(s in h)if(h.hasOwnProperty(s)){var q=$("<option />",{value:h[s]});f.append(q)}e.after(f)}}else 1<e.length&&a.notice("Unexpected 1 field, got "+e.length)}}})):c.after(b)}),d=$("<li />"),d.append(b),$("table.variants tr.variant td:last-child ul.segmented").append(d))},build_select:function(b,c){if(b){var d=
a.data("collections"),e=$("<select />",{"data-ref":b,style:"width:50%"}).change(function(a){var c=$(this);a=c.val();c=c.parent().find('input[name="'+b+'"]').eq(0);a?c.val(a):c.val("")});if(d&&1<Object.keys(d.collections).length){for(var f='<option value="">Pick a collection</option>',h=0,s=Object.keys(d.collections).length;h<s;h++)f+='<option value="'+d.collections[h].handle+'">'+d.collections[h].title+"</option>";e.html(f);return e}a.notice("Error, invalid collection length",!0)}},type_check:function(b,
